...
@if(Auth::user())
    ...
    <div class="row justify-content-center mt-2">
        <form action="" method="post">
            ...
            <button class="btn" style="background-color:transparent;" name="daily_id" value="{{ $daily->id }}" @if($isDisabled) disabled @endif>
                <img src="{{ $daily->harvestImageUrl }}" class="w-100" style="max-width:200px;" />
            </button>
        </form>
    </div>
    @else
    <div class="row justify-content-center mt-2">
        <form action="" method="post">
            @csrf
            <button class="btn btn-success" name="daily_id" value="{{ $daily->id }}" @if($isDisabled) disabled @endif><img src="{{ $daily->harvestImageUrl }}" style="max-width:400px"></button>
        </form>
    </div>
    @endif
    <div class="text-center">
